    ...
    useEffect(() => {
        if (searchTerm.trim()) {
            const filtered = parsedStocksArray.filter(stock => {
                const stockName = stock.stock_name; // Access the stock_name property
                return stockName.toLowerCase().includes(searchTerm.toLowerCase());
            });
            setFilteredStocks(filtered);
        } else {
            setFilteredStocks(parsedStocksArray);
        }
    }, [searchTerm]);
                ...
                <>
                    <div className="mt80" style={{ zIndex: 1000 }}>
                        <SASidebar
                            ...
                            ffilteredStocks={filteredStocks.map(stock => stock.stock_name)}
                            email={email}
