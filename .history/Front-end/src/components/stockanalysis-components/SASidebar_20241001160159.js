...
const SASidebar = ({ favouriteStocks, addFavourite, removeFavourite, onSearch, onNavigate = () => { } }) => {
    ...
    const toggleFavourite = (companyTitle, isRemoving) => {
        if (isRemoving) {
            // Logic to remove from favorites
            console.log(`Removing ${companyTitle} from favorites`);
            setFavourites((prevFavourites) => prevFavourites.filter(title => title !== companyTitle));
        } else {
            // Logic to add to favorites
            console.log(`Adding ${companyTitle} to favorites`);
            setFavourites((prevFavourites) => [...prevFavourites, companyTitle]);
        }
    };  
    
    // Check if a company is in the favorites
    const isFavourite = (companyTitle) => {
        return favourites.includes(companyTitle);
    };
                                            ...
                                            {index === 0 && (
                                                <>
                                                    <SASidebarCard
                                                        ...
                                                        onFavourite={() => toggleFavourite('BEGA CHEESE LIMITED (BGA)', false)} // Adding to favorites
                                                        onRemoveFavourite={() => toggleFavourite('BEGA CHEESE LIMITED (BGA)', true)} // Removing from favorites
                                                        isFavourited={isFavourite('BEGA CHEESE LIMITED (BGA)')} // Check if it’s a favorite
                                                    />
                                                    <SASidebarCard
