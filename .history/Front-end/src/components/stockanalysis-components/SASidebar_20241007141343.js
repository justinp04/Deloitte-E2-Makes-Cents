...
const SASidebar = ({ favouriteStocks, addFavourite, removeFavourite, addFavouriteToDatabase, onSearch, onNavigate = () => { } }) => {
    ...
    useEffect(() => {
        // Filter stock suggestions based on the search term
        const filteredSuggestions = parsedStocksArray.filter(stock =>
                                                ...
                                                <>
                                                    {stockSuggestions.length > 0 ? (
                                                        stockSuggestions.map((stock, idx) => (
                                                            <SASidebarCard
                                                                key={idx}
                                                                companyTitle={stock.stock_name} // Adjusted here
                                                                onClick={() => onNavigate(stock.stock_name)}
                                                                onFavourite={() => addFavourite(stock.ticker)} // Assuming you want to use the ticker for favouriting
                                                            />
                                                        ))
                                                    ) : (
                                                ...
                                                <>
                                                    ...
                                                    {stockSuggestions.length > 0 ? (
                                                        stockSuggestions.map((stock, idx) => (
                                                            <SearchCard
                                                                key={idx}
                                                                companyTitle={stock} // Display the company name
                                                                onClick={() => handleSuggestedItemClick(stock)} // Click to navigate
                                                            />
                                                        ))
                                                    ) : (
