...
const SASidebar = ({ favouriteStocks, addFavourite, removeFavourite, addFavouriteToDatabase, onSearch, onNavigate = () => { } }) => {
    ...
    const handleSearchChange = (term) => {
        setSearchTerm(term);
        if (term.trim()) {
            // Filter the complete stock list based on the search term
            const filteredStocks = allStocks.filter(stock => 
                stock.toLowerCase().includes(term.toLowerCase())
            );
            setStockSuggestions(filteredStocks); // Update the stock suggestions
        } else {
            setStockSuggestions([]); // Reset when input is empty
        }
    };
