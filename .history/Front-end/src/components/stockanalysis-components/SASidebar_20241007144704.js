...
const SASidebar = ({ favouriteStocks, addFavourite, removeFavourite, addFavouriteToDatabase, onSearch, onNavigate = () => { } }) => {
    ...
    const handleSearchChange = (term) => {
        ...
        if (term.trim()) {
            ...
            setStockSuggestions(filteredStocks); 
        } else {
            setStockSuggestions([]);
        }
    };
                                ...
                                <Accordion.Item eventKey={key} key={key}>
                                    ...
                                    {expandedItems[key] && (
                                        <Accordion.Body>
                                            ...
                                            {index === 2 && (
                                                <>
                                                <SearchBar placeholder="Search for a stock" onSearch={handleSearchChange} />
                                                {searchTerm && stockSuggestions.length > 0 ? (
                                                    stockSuggestions.map((result, idx) => (
                                                        <SearchCard
                                                            key={idx}
                                                            companyTitle={result}  // Display the filtered stock name
                                                            onClick={() => onNavigate(result)} // Navigate when clicked
                                                        />
                                                    ))
                                                ) : (
