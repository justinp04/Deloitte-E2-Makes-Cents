...
const StockSummary = ({ summary, references, accordionOpen, setAccordionOpen, addFavourite, removeFavourite, favouriteStocks, responseDepth, onToggleChange, email, stockName}) => {
    ...
    const [initStockName, setStockName] = useState(stockName || '');
    ...
    useEffect(() => {
        const fetchStockData = async () => {
            ...
            try {
                const res = await fetch('http://localhost:4000/summary/stock-summary', {
                    ...
                    body: JSON.stringify({ initStockName, user_email: email, response_depth: responseDepth })
                });
        ...
        if (initStockName && initStockName !== "Unknown") {
            fetchStockData();
        }
    
    }, [initStockName, email, responseDepth]);

    // Compute the company title including stock ticker
    const companyTitle = initStockName && stockTicker ? `${initStockName} (${stockTicker})` : "No stock name provided";
