...
API_URL ='http://localhost:4000/news?symbol=BHP'
...
def get_fav_stock():
    ...
    fav_stocks = ["WOW", "AFI"] #this is just for testing purposes until it gets hooked up to sql
...
def query_qdrant_for_news(stock):
    ...
    past_date = (datetime.datetime.now() - datetime.timedelta(days=30)).strftime('%Y-%m-%d')  # Get the date 30 days ago

    # Query Qdrant for news related to the stock and within the last 30 days
    user_query = f"{stock} news between {past_date} and {today}"
