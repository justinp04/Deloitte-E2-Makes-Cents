...
# Function to fetch user profile from the /user-responses endpoint
def fetch_user_profile(user_id):
    try:
        response = requests.get(USER_RESPONSE_URL, params={"user_id": user_id})
        response.raise_for_status()  # Raise error if the request fails
        user_profile = response.json()  # Parse the JSON response
...
def main(user_id, stock):
    # Step 1: Fetch the user responses from the /user-responses API
...
if __name__ == "__main__":
    main(user_id=9, stock="BHP")  # Example usage
