...
def main(user_email, stock):
    ...
    summaries = generate_gpt3_summary(articles_with_summaries, user_profile)
...
# Function to generate GPT-3.5 article summary
def generate_article_summaries(articles):
    articles_with_summaries = []
    for article in articles:
        article_title = article.get('article_title', 'No Title Available')
        article_url = article.get('article_url', 'URL not available')

        # Generate summary using GPT-3.5 (pseudo-code, actual API call depends on your GPT-3.5 setup)
        messages = [
            {"role": "system", "content": "Please provide a brief summary of the following article."},
            {"role": "user", "content": f"Title: {article_title}\nURL: {article_url}"}
        ]

        try:
            response = get_llm_response(messages, max_response_tokens=100)
            summary = response.choices[0].message.content.strip()
