...
'''''''''''''''''''''''''''''''''''''''''''''''''''
def get_stock_name1(user_input, asx_stocks):
...
def main():
    # Load ASX stocks from the CSV
    print("Loading ASX stocks...")
    asx_stocks = load_asx_stocks()
    if not asx_stocks:
        print("Failed to load ASX stocks.")
        return
    
    # Print the loaded stocks
    print(f"Loaded {len(asx_stocks)} stocks.")
    for name, ticker in list(asx_stocks.items())[:5]:  # Show first 5 stocks as a sample
        print(f"Stock Name: {name}, Ticker: {ticker}")
    
    # Ask user for input to test stock name searching
    user_input = input("\nEnter the stock name or ticker to search for: ").strip()
    
    # Get the stock ticker based on user input
    stock_data = get_stock_name1(user_input, asx_stocks)
    if stock_data:
        print(f"\nStock found: {stock_data['name']} with ticker: {stock_data['ticker']}")
    else:
