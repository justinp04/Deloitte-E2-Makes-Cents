...
def generate_response(documents, user_query):
    ...
    cleaned_response = clean_summary_text(response.choices[0].message.content.strip())
    formatted_response = format_detailed_summary(cleaned_response)
...
def format_detailed_summary(detailed_answer):
    ...
    """
    # Replace "\n\n[number]\n\n" with just "\n[number] " to prevent extra line breaks
