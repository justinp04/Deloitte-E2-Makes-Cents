...
def format_detailed_summary(detailed_answer):
    ...
    replacing "\n\n[number]\n\n" or "\n[number]\n" with "[number]. ".
    """
    # First, remove newlines around numbered items like 1., 2., 3.:
    formatted_summary = re.sub(r'\n{2}(\d+)\.\n{2}', r' \1. ', detailed_answer)
    
    # To catch other possible patterns, like a single newline before and after a number:
    formatted_summary = re.sub(r'\n(\d+)\.\n', r' \1. ', formatted_summary)

    # Optionally remove any multiple consecutive newlines that may still remain:
    formatted_summary = re.sub(r'\n+', '\n', formatted_summary)
